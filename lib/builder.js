...
function ensureElmPackageExists(config, elmPackagePath, location) {
  return new bluebird.Promise(function(resolve, reject) {
    ...
    if (!config.prompt) {
      runElmPackageInstall(config.compiler, path.dirname(elmPackagePath), false, resolve, reject);
      return;
    }
    ...
    promptly.confirm('Unable to find elm-package.json in the ' + location + ' directory' +
      ...
      if (err) {
        ...
        runElmPackageInstall(config.compiler, path.dirname(elmPackagePath), true, resolve, reject);
      } else {
        reject();
      }
    });
...
function addSourceDirectories(additions, additionDir, testElmPackagePath, sourceDirectories) {
  ...
  var relativeSourceDirectories = _.map(additions, function(p) {
    return path.join(relativePath, p);
  }).filter(function(p) {
...
function installDependencies(config, testDirectory) {
  return new bluebird.Promise(function(resolve, reject) {
    runElmPackageInstall(config.compiler, testDirectory, config.prompt, resolve, reject);
  });
}

function runElmPackageInstall(compiler, directory, prompt, resolve, reject) {
  var command = 'elm-package';

  if (compiler) {
    command = path.join(compiler, command);
  }
